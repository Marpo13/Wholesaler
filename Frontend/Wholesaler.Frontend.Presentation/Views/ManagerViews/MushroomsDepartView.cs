...
namespace Wholesaler.Frontend.Presentation.Views.ManagerViews
{
    internal class MushroomsDepartView : View
    {
        ...
        public MushroomsDepartView(IRequirementRepository requirementRepository, ApplicationState state) : base(state)
        {
        ...
        protected async override Task RenderViewAsync()
        {
            ...
            {
                var getRequirement = await _requirementRepository.GetAllRequirements();
                ...
                var departure = await _requirementRepository.CompleteRequirement(requirement.Id);
