...
namespace Wholesaler.Frontend.Presentation.Views
{
    internal class MenuView : View, IMenuView
    {
        ...
        public MenuView(ApplicationState state, EmployeeView employeeView, ManagerView managerView, OwnerView ownerView) : base(state)
        {
        ...
        protected override async Task RenderViewAsync()
        {
            ...
            switch (role)
            {
                ...
                default: throw new Exception($"User login: {State.GetLoggedInUser().Login} has invalid role: {role}.");
            }
        }
    }
}
